package com.bero.library.opengles;
...
public class GLSurfaceView extends SurfaceView implements SurfaceHolder.Callback{
	...
	private int mEGLContextClientVersion=1;
	 ...
	 *            The current context
	 */
	public boolean checkGLES20Support(Context context){
	...
	/**
	 * Get the OpenGL ES glGetString strings as a string array (GL_VENDOR,GL_RENDERER,GL_VERSION,GL_EXTENSIONS)
	 * 
	 * @param Context
	 *            The current context
	 */
	public String[] getGLRenderer(Context context){
		EGL10 egl=(EGL10)EGLContext.getEGL();
		EGLDisplay display=egl.eglGetDisplay(EGL10.EGL_DEFAULT_DISPLAY);
		...
		int[] configAttribs;
		configAttribs=new int[]{EGL10.EGL_DEPTH_SIZE,0,EGL10.EGL_STENCIL_SIZE,0,EGL10.EGL_RED_SIZE,5,EGL10.EGL_GREEN_SIZE,6,EGL10.EGL_BLUE_SIZE,5,EGL10.EGL_ALPHA_SIZE,0,EGL10.EGL_SURFACE_TYPE,EGL10.EGL_PBUFFER_BIT,EGL10.EGL_NONE};
		...
		egl.eglChooseConfig(display,configAttribs,configs,10,num_config);
		if(num_config[0]>0){
			EGLContext mEGLContext=egl.eglCreateContext(display,configs[0],EGL10.EGL_NO_CONTEXT,null);
			if(mEGLContext!=null){
				int[] ContextAttribs={EGL10.EGL_WIDTH,256,EGL10.EGL_HEIGHT,256,EGL10.EGL_NONE};
				EGLSurface surface=egl.eglCreatePbufferSurface(display,configs[0],ContextAttribs);
				if(surface!=null){
					egl.eglMakeCurrent(display,surface,surface,mEGLContext);
					GL10 gl=(GL10)mEGLContext.getGL();
					if(gl!=null){
						s[0]=gl.glGetString(GL10.GL_VENDOR);
						s[1]=gl.glGetString(GL10.GL_RENDERER);
						s[2]=gl.glGetString(GL10.GL_VERSION);
						s[3]=gl.glGetString(GL10.GL_EXTENSIONS);
					}
					egl.eglMakeCurrent(display,EGL10.EGL_NO_SURFACE,EGL10.EGL_NO_SURFACE,EGL10.EGL_NO_CONTEXT);
					egl.eglDestroySurface(display,surface);
				}
				egl.eglDestroyContext(display,mEGLContext);
			}
		}
		egl.eglTerminate(display);
		return s;
	...
	public void setRenderer(Renderer renderer){
		...
		if(mEGLConfigChooser==null){
			mEGLConfigChooser=(mEGLContextClientVersion==2)?new GLES20SimpleEGLConfigChooser(true):new SimpleEGLConfigChooser(true);
		}
		if(mEGLContextFactory==null){
			mEGLContextFactory=(mEGLContextClientVersion==2)?new GLES20ContextFactory():new DefaultContextFactory();
		}
	...
	public void setEGLConfigChooser(boolean needDepth){
		setEGLConfigChooser((mEGLContextClientVersion==2)?new GLES20SimpleEGLConfigChooser(needDepth):new SimpleEGLConfigChooser(needDepth));
	}
	...
	public void setEGLConfigChooser(int redSize,int greenSize,int blueSize,int alphaSize,int depthSize,int stencilSize){
		setEGLConfigChooser((mEGLContextClientVersion==2)?new GLES20ComponentSizeChooser(redSize,greenSize,blueSize,alphaSize,depthSize,stencilSize):new ComponentSizeChooser(redSize,greenSize,blueSize,alphaSize,depthSize,stencilSize));
	}
	 ...
	 * 
	 * Note: Activities which require OpenGL ES 2.0 should indicate this by setting <uses-feature android:glEsVersion="0x00020000" /> in the activity's AndroidManifest.xml file.
	 * 
	 ...
	 * There are situations where the EGL rendering context will be lost. This typically happens when device wakes up after going to sleep. When the EGL context is lost, all OpenGL resources (such as textures) that are associated with that context will be automatically deleted. In order to keep rendering correctly, a renderer must recreate any lost resources that it still needs. The {@link #onSurfaceCreated(GL10, EGLConfig)} method is a convenient place to do this.
	 * 
	 * 
	...
	public static class GLES20ContextFactory implements GLSurfaceView.EGLContextFactory{
		private static final String TAG="GLES20ContextFactory";
	...
	public interface EGLConfigChooser{
		/**
		 * Choose a configuration from the list. Implementors typically implement this method by calling {@link EGL10#eglChooseConfig} and iterating through the results. Please consult the EGL specification available from The Khronos Group to learn how to call eglChooseConfig.
		 * 
		 * @param egl
	...
	public static class GLES20ComponentSizeChooser implements EGLConfigChooser{
		private static final String TAG="GLES20ConfigChooser";

		public GLES20ComponentSizeChooser(int r,int g,int b,int a,int depth,int stencil){
			super();
			mRedSize=r;
			mGreenSize=g;
			mBlueSize=b;
	...
	private static class GLES20SimpleEGLConfigChooser extends GLES20ComponentSizeChooser{
		public GLES20SimpleEGLConfigChooser(boolean withDepthBuffer){
			super(4,4,4,0,withDepthBuffer?16:0,0);
			// Adjust target values. This way we'll accept a 4444 or
			// 555 buffer if there's no 565 buffer available.
			mRedSize=5;
